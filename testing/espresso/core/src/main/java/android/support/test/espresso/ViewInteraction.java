...
public final class ViewInteraction {
  ...
  //mf: added to access doPerformNew
  public ViewInteraction performNew(final ViewAction... viewActions) {
    checkNotNull(viewActions);
    for (ViewAction action : viewActions) {
      doPerformNew(action);
    }
  ...
  //mf: added (main difference with doPerform is that it does not request to run on the main thread because it is already on the main thread)
  private void doPerformNew(final ViewAction viewAction) {
    checkNotNull(viewAction);
    final Matcher<? extends View> constraints = checkNotNull(viewAction.getConstraints());
    uiController.loopMainThreadUntilIdle();
    View targetView = viewFinder.getView();
