...
public class Hierarchy {
  ...
  void recordFrameworkInvokedCallbacks() {
    // (1) possible callbacks from the framework to app classes,
    ...
    for (SootClass c : appClasses) {
      ...
      //mf: comment this is a limitation
...
//    //mf: logging
//    Set<String> appPackageSet = new HashSet<String>();
//    for(SootClass appActivityClass:applicationActivityClasses){
//    	if(appActivityClass!=null){
//    		appPackageSet.add(appActivityClass.getPackageName());
//    	}
//    }
//    for(String appPackage:appPackageSet){
...
//    Configs.addLibraryPackage("com.google.android.gms.*");
    Set<String> libPackageSet = new HashSet<String>();
    for(SootClass libActivityClass:libActivityClasses){
    	if(libActivityClass!=null){
    		//because of code in AnalysisEntrypoint:run
    		if(libActivityClass.getPackageName().startsWith("android.support") || libActivityClass.getPackageName().startsWith("com.google.android.gm")){
    			continue;
    		}
    		libPackageSet.add(libActivityClass.getPackageName());
    	}
    }
    String libPackages = "";
    for(String libPackage:libPackageSet){
    	libPackages = libPackages + "," + libPackage;
    }
    System.out.println("activity lib packages to add:"+libPackages);
  }
  ...
  void viewsAndMenus() {
    ...
    for (SootClass c : appClasses) {
      ...
      if (superClzSet.contains(view)) {
		...
		//mf: added
    	if(c!=null){
    		appPackageSet.add(c.getPackageName());
    	}
      }
    ...
    for (SootClass c : scene.getLibraryClasses()) {
      ...
      if (superClzSet.contains(view)) {
		...
		//mf: added
    	if(c!=null){
    		if(c.getPackageName().startsWith("android.support") || c.getPackageName().startsWith("com.google.android.gm")){
    			//do nothing
    		}
    		else{
    			libPackageSet.add(c.getPackageName());
    		}
    	}
      }
      if (superClzSet.contains(menuItem)) {
...
//    //mf: added
//    for(String appPackage:appPackageSet){
//    	System.out.println("app view:"+appPackage);
//    }    
    String libPackages = "";
    for(String libPackage:libPackageSet){
    	libPackages = libPackages + "," + libPackage;
    }
    System.out.println("view lib packages to add:"+libPackages);
