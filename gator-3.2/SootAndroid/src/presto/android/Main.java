...
public class Main {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if ("-project".equals(s)) {
        ...
        Configs.resourceLocation = args[++i];
          ...
          Configs.ontologyFileName = args[++i];
      } else if ("-string".equals(s)) {
          Configs.stringFileName = args[++i];
      } else if ("-include".equals(s)) {
          Configs.include = args[++i];
      } else if ("-exclude".equals(s)) {
          Configs.exclude = args[++i];
      } else {
  ...
  static void setupAndInvokeSoot() {
    ...
    String apiLevel = Configs.apiLevel;
    apiLevel = apiLevel.substring(apiLevel.lastIndexOf("-")+1);
    ...
    //      "-coffi",
      ...
      //mf: added to improve speed of analysis
      //included packages
      String includedString = Configs.include;
	  if(!includedString.equals("")){
	      List<String> includedList = new ArrayList<String>();
	      String[] includedArray = includedString.split(",");
	      for(String included:includedArray){
	    	  includedList.add(included);
	      }
	      Scene.v().setIncludedPackages(includedList);
      }
	  else{
		  Scene.v().setIncludeAll(true);
	  }
	  //excluded packages
	  String excludedString = Configs.exclude;
	      if(!excludedString.equals("")){
	      List<String> excludeList = new ArrayList<String>();      
	      String[] excludedArray = excludedString.split(",");
	      for(String excluded:excludedArray){
	    	  excludeList.add(excluded);
	      }
	      Scene.v().setExcludedPackages(excludeList);
      }
