...
//	//mf: logging
//	  System.out.println("analyzeCallbackMethod:widget:"+guiWidget);
//	  System.out.println("analyzeCallbackMethod:handler:"+handler);
      ...
      public boolean match(Stmt unit, SootMethod context) {
        ...
        if (stmtFilter.lookforOpenMenu()) {
          if (wtgUtil.isMenuItemAddCall(unit)
            ...
            } else if (handler.getSubSignature().equals(MethodNames.sherlockOnCreateOptionsMenuSubsig)
                    && collectTargetMenu(unit).contains(guiWidget)) {
                openMenus.put(guiWidget, new Pair<Stmt, SootMethod>(unit, context));
...
//	  //mf: logging
//	  System.out.println("addStartActivityTarget:widget:"+guiWidget);
//	  System.out.println("addStartActivityTarget:handler:"+handler);
