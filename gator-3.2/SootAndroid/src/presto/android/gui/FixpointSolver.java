...
public class FixpointSolver {
  ...
  public void solveCore() {
	System.out.println("*****************Fixpoint START*****************");
    // compute
	System.out.println("###############layoutIdReachability##################");
	//printState();
    layoutIdReachability();
    System.out.println("###############menuIdReachability##################");
    //printState();
    menuIdReachability();

    System.out.println("###############windowReachability##################");
    //printState();
    windowReachability();
    System.out.println("###############optionsMenuReachability##################");
    //printState();
    optionsMenuReachability();
    System.out.println("###############contextMenuReachability##################");
    //printState();
    contextMenuReachability();
    System.out.println("###############viewIdReachability##################");
    //printState();
    viewIdReachability();

    System.out.println("###############computePathsFromViewProducerToViewConsumer##################");
    //printState();
    computePathsFromViewProducerToViewConsumer();

    // process inflater calls
    System.out.println("###############processInflaterCalls##################");
    //printState();
    processInflaterCalls();

    // propagation
    System.out.println("###############viewAndListenerPropagation##################");
    //printState();
    viewAndListenerPropagation();
    System.out.println("*****************Fixpoint END*****************");
  }
...
//        Set<NIdNode> viewIds = reachingViewIds.get(source);
//        if(viewIds!=null){
//      	  for(NIdNode nodeId:viewIds){
//      		  if(nodeId.getIdName().equals("settings")){
//      			  isSettings = true;
//      			  break;
//      		  }
//      	  }
...
//        }
      ...
      for (NNode target : graphUtil.reachableNodes(layoutIdNode)) {
        ...
        if (opNode.consumesLayoutId()) {
          ...
          if (Configs.sanityCheck) {
            ...
            Logger.verb("WARNING", layoutIdNode + " reaching " + opNode + " in layoutIdReachability" +
                    " method");
          }
        }
      }
  ...
  void menuIdReachability() {
    for (NMenuIdNode menuIdNode : flowgraph.allNMenuIdNodes.values()) {
    	//mf: logging
    	//System.out.println("menu:"+menuIdNode.getIdName());
  ...
  void optionsMenuReachability() {
    for (NOptionsMenuNode optionsMenu : flowgraph.activityClassToOptionsMenu.values()) {
    	//mf: logging
    	//System.out.println("OptionsMenu:"+optionsMenu.toString());
      for (NNode target : graphUtil.reachableNodes(optionsMenu)) {
        ...
        if (opNode instanceof NFindView1OpNode
            ...
            || opNode instanceof NAddView3OpNode
          ...
          //mf: logging
          //System.out.println("Target:"+opNode.toString());
        } else {
...
//    	//mf: logging
//    	boolean log=false;
//    	if(viewIdNode.getIdName().equals("settings")){
//    		log=true;
//    		System.out.println("viewIdReachability:node id:"+viewIdNode.getIdName());
//    	}
...
//	  	//mf: logging
//	  	if(log){
//	  		System.out.println("viewIdReachability:reachable node:"+target.toString());
//	  	}
...
//	          //mf: logging
//	          if(log){
//	        	  System.out.println("viewIdReachability:reaching view ids:"+viewIdNode.toString());
//	          }
...
//    	//mf: logging
//    	System.out.println("viewIdReachability:anonymous id node:"+anonymousIdNode.toString());
      ...
      for (NNode target : graphUtil.reachableNodes(anonymousIdNode)) {
        ...
        if (opNode instanceof NFindView1OpNode) {
//        	//mf: logging
//        	System.out.println("viewIdReachability:anonymous op node:"+opNode.toString());
  ...
  void parameterAndReceiverViewReachability(NNode source) {
    ...
    for (NNode target : reachables) {
      ...
      if (opNode instanceof NFindView1OpNode
          ...
          || (opNode instanceof NAddView3OpNode && reachables.contains(opNode.getReceiver()))) {
  ...
  void recordViewProducers(NNode objectNode, NOpNode producerNode) {
	  ...
	  //System.out.println("Producer:"+producerNode.toString());
  ...
  void processInflaterCalls() {
    ...
    System.out.println("$$$$$$$$$$$$$$$processViewInflaterCalls$$$$$$$$$$$$$$$$");
    processViewInflaterCalls();
    
    System.out.println("$$$$$$$$$$$$$$$processMenuInflaterCalls$$$$$$$$$$$$$$$$");
...
//    	  //mf: logging
//    	  if(layoutIdNode.toString().contains("layout_drawer_loggedin")){
//    		  System.out.println("LayoutId:"+layoutIdNode.toString());
//    		  System.out.println("OpNode:"+opNode.toString());
//    		  System.out.println("InflNode:"+inflNode.toString());
//    	  }
          
          //mf: comment IMPORTANT here it is possible to make a connection between Layout id and generated view 
...
//      	  //mf: logging
//      	  if(layoutIdNode.toString().contains("layout_activity_overview")){
//      		  System.out.println("LayoutId:"+layoutIdNode.toString());
//      		  System.out.println("OpNode:"+opNode.toString());
//      		  System.out.println("InflNode:"+root.toString());
//      	  }
  ...
  void inflNodeToReceiverAndParameter(NNode inflNode) {	  
    ...
    for (NNode n : reachables) {
      ...
      if (opNode instanceof NAddView1OpNode
          ...
          || (opNode instanceof NAddView3OpNode && reachables.contains(opNode.getParameter()))) {
...
//    for(String key:root.getAttrs().keySet()){
//        System.out.println(key+"#"+root.getAttrs().get(key));
//    }
...
//        vNode = flowgraph.inflNode(v.getSootClass());
      ...
      //mf: added
      vNode.setResourceIdReference(v.getResourceIdReference());
      vNode.setResourceIdValue(v.getResourceIdValue());
      vNode.setTextReference(v.getTextReference());
      vNode.setTextValue(v.getTextValue());
      vNode.setImageReference(v.getImageReference());
  ...
  void viewAndListenerPropagation() {
    ...
    while (true) {
      ...
      for (NOpNode addView3 : NOpNode.getNodes(NAddView3OpNode.class)) {
          if (processAddView3((NAddView3OpNode)addView3)) {
            changed = true;
        }
      }
...
//    //mf: logging
//    System.out.println("processFindView1:node:"+node.toString());
//    for(NNode r:receivers)
//    	System.out.println("processFindView1:solutionReceivers:"+r.toString());
...
//    	System.out.println("processFindView1:solutionResults:"+s.toString());
    
    int oldSize = solution.size();
    for (NNode receiver : receivers) {
    	
//    	//mf:logging
//    	System.out.println("processFindView1:solutionReceiver:"+receiver.toString());
...
//      	System.out.println("processFindView1:descendants:"+d.toString());
...
//    //mf: logging
//    System.out.println("processAddView2:op:"+node.toString());
//    for(NNode parent:parentSet){
//    	System.out.println("processAddView2:receivers:"+parent.toString());
//    }
//    for(NNode child:childSet){
//    	System.out.println("processAddView2:parameters:"+child.toString());
//    }
  ...
  boolean processAddView3(NAddView3OpNode node) {
    boolean changed = false;
    Set<NNode> parentSet = solutionReceivers.get(node);
    if (parentSet == null || parentSet.isEmpty()) {
      return false;
...
//    //mf: logging
//    System.out.println("processAddView3:op:"+node.toString());
//    for(NNode parent:parentSet){
//    	System.out.println("processAddView3:receivers:"+parent.toString());
//    }
//    for(NNode child:childSet){
//    	System.out.println("processAddView3:parameters:"+child.toString());
//    }
    ...
    for (NNode parent : parentSet) {
      for (NNode child : childSet) {
        if (parent == child) {
          if (Configs.verbose) {
            System.out.println(
                "[WARNING] p.addHeaderView(p) for " + node + "\np=" + parent);
          }
          continue;
        }
        if (!parent.hasChild(child)) {
      ...
      for (NOpNode call : callsWithParameter) {
        ...
        for (NNode s : solution) {
          if (isValidFlowByType(s, call, VarType.Parameter)) {
            trueSolution.add(s);
//            //mf: logging
//            if(call.toString().contains("addHeaderView")){
//            	System.out.println("propagateViewToParameter:op:"+call.toString());
//            	System.out.println("propagateViewToParameter:parameter:"+s.toString());
//            }
            if (Configs.debugCodes.contains(Debug.WORKLIST_DEBUG)) {
