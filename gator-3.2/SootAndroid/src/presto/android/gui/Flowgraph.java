...
import presto.android.gui.graph.NAddView3OpNode;
...
//	//mf: logging
//	System.out.println("activity:"+activityClass.toString());
    ...
    //mf: added to support sherlock
    matched = hier.matchForVirtualDispatch(
        sherlockOnCreateOptionsMenuSubsig, activityClass);
    if (matched != null && matched.isApplicationClass()) {
      menuCallbacks.add(matched.getMethod(sherlockOnCreateOptionsMenuSubsig));
    }
    matched = hier.matchForVirtualDispatch(
    ...
    matched = hier.matchForVirtualDispatch(
        sherlockOnPrepareOptionsMenuSubsig, activityClass);
    if (matched != null && matched.isApplicationClass()) {
      menuCallbacks.add(matched.getMethod(sherlockOnPrepareOptionsMenuSubsig));
    }
...
//	//mf: logging
//	System.out.println("options node:"+optionsMenuNode.toString());
    
    SootClass menuClass = Scene.v().getSootClass("android.view.Menu");
    Set<String> itemSelectedSubsigs = Sets.newHashSet(
        onOptionsItemSelectedSubSig, sherlockOnOptionsItemSelectedSubSig, onMenuItemSelectedSubSig);//mf: changed
    for (SootMethod cb : menuCallbacks) {
//    	//mf: logging
//    	System.out.println("callback:"+cb.toString());
    	
      Local menuLocal = jimpleUtil.localForNthParameter(cb, 1);
      
//  		//mf: logging
//  		System.out.println("menu local:"+menuLocal.toString());
...
//      	System.out.println("tgt:"+tgt.toString());
      ...
      for (Iterator<SootMethod> iter = Lists.newArrayList(c.getMethods()).iterator(); iter
        ...
        while (stmts.hasNext()) {
          ...
          if (currentStmt.containsInvokeExpr()) {
            ...
            NOpNode opNode = null;
            try {
              opNode = createOpNode(currentStmt, b);
            } catch (Exception e) {
            ...
            if (ie instanceof StaticInvokeExpr
              ...
              if (stm.getDeclaringClass().isApplicationClass()) {
                  //mf: logging
                  //System.out.println("Process flow at call:"+currentStmt.toString());
          ...
          if (lhs instanceof ArrayRef) {
            ...
            if (x instanceof Local) {
            	//mf: logging
                //System.out.println("Record var at array ref write:"+currentStmt.toString());
          ...
          if (rhs instanceof ArrayRef) {
            ...
            if (x instanceof Local) {
            	//mf: logging
                //System.out.println("Record var at array ref read:"+currentStmt.toString());
          ...
          if (nn_lhs != null && nn_rhs != null) {
        	//mf: logging
              //System.out.println("Add edge:"+currentStmt.toString());
  ...
  public void build() {
	System.out.println("*****************Flow graph START*****************");
    buildIdNodes();
    System.out.println("###############processFrameworkManagedCallbacks##################");
    processFrameworkManagedCallbacks();
    System.out.println("###############processApplicationClasses##################");
    //mf: comment this method creates links between listeners and views
    ...
    // find it necessary
    System.out.println("###############resolveArrayRefs###############");
    resolveArrayRefs();

    // Deal with recorded dialog and its builder calls
    // WARNING: the order of the following two calls cannot be changed!!!
    System.out.println("###############processAllRecordedDialogCalls###############");
    processAllRecordedDialogCalls();

    System.out.println("###############checkAndPatchRootlessActivities###############");
    checkAndPatchRootlessActivities();
    System.out.println("###############checkAndPatchRootlessDialogs###############");
    checkAndPatchRootlessDialogs();

    // For each ListActivity, model its onListItemClick
    System.out.println("###############patchListActivity###############");
    patchListActivity();

    // Deal with list views and list adapters
    System.out.println("###############processRecordedListViewCalls###############");
    processRecordedListViewCalls();

    // TabHost, TabSpec...
    System.out.println("###############processTabHostRelatedCalls###############");
    processTabHostRelatedCalls();

    System.out.println("###############processFlowFromSetListenerToEventHandlers###############");
    processFlowFromSetListenerToEventHandlers();
    System.out.println("*****************Flow graph END*****************");
    
    //saving ontology
    if(xmlUtil instanceof DefaultXMLParser){
    	System.out.println("*****************Saving ontology*****************");
    	DefaultXMLParser dxp = (DefaultXMLParser) xmlUtil;
    	generateOntology(dxp.getAndroidViewSet());
    	if(dxp.getAndroidViewSet().size()>0){
    		System.exit(0);
    	}
    }
  }
  
  private JSONArray ontologyJSONArray = new JSONArray();
  
  public void generateOntology(Set<AndroidView> androidViewSet){
	  for(AndroidView aw:androidViewSet){
		  saveOntology("", "", new ArrayList<String>(), new ArrayList<String>(),
				  aw.getText(), new ArrayList<String>(), new ArrayList<String>(), "", new ArrayList<String>(), new ArrayList<String>(),
				  "", aw.getResourceIdReference(), new ArrayList<String>(), new ArrayList<String>(),
				  aw.getImageReference(), new ArrayList<String>(), new ArrayList<String>());
	  }
	  
		try{
			JSONObject ontologyJSON = new JSONObject();
			SootClass mainClz = xmlUtil.getMainActivity();
			...
			//String fileName = Configs.ontologyFileName.replace("ontology.json", "ontology_new.json");
		...
		try{
			...
			Map<String, String> stringIdValueMap = xmlUtil.getStringIdValueMapping();
			Map<String, String> sysStringIdValueMap = xmlUtil.getSysStringIdValueMapping();
			...
			//String fileName = Configs.stringFileName.replace("string.json", "string_new.json");
  ...
  private void saveOntology(String fullClassName, String className, List<String> classNameKeywordsList, List<String> classNameLemmasList,
			...
			try{
				...
				ontologyJSONArray.put(nodeOntologyJSON);
			}
  ...
  // Op Nodes
  public NOpNode createOpNode(Stmt s, Body b) {
    ...
    // FindView1: view.findViewById(id)
    {
      ...
      if (findView1 != null) {
//    	 //mf: logging
//    	System.out.println("createOpNode:processed:"+s.toString());
        return findView1;
      }
    }
    ...
    {
      NOpNode addView3 = createAddView3OpNode(s);
      if (addView3 != null) {
        return addView3;
      }
    }
    ...
    {
      NOpNode addMenuItem = createAddMenuItemOpNode(s, b);
      if (addMenuItem != null) {
        return addMenuItem;
      }
    }
...
////  //mf: logging
//      System.out.println("lhs node"+lhsNode);
//    System.out.println("fake local node:"+fakeLocal);
//    System.out.println("inflate node:"+inflate1);
//    System.out.println("outside root node:"+outsideRootNode);
//    System.out.println("add view node:"+addView2);
...
//      //mf: logging
//      System.out.println("layoutIdNode:"+layoutIdNode);
//      System.out.println("receiverNode:"+receiverNode);
//      System.out.println("statement:"+receiverNode);
//      System.out.println("lookup:"+jimpleUtil.lookup(s));
  ...
  //AddView3: parent.addHeaderView(child, data, isSelectable)
  //AddView3: parent.addHeaderView(child)
  public NAddView3OpNode createAddView3OpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = ie.getMethod();
    SootClass c = callee.getDeclaringClass();
    String name = callee.getName();
    boolean match = name.equals(addHeaderViewName) && hier.viewClasses.contains(c);
    if (!match) {
      return null;
    }
    ...
    NVarNode parentNode = varNode(jimpleUtil.receiver(ie));
    NVarNode childNode = varNode((Local)ie.getArg(0));
    return new NAddView3OpNode(parentNode, childNode,
        new Pair<Stmt, SootMethod>(s, caller), false);
  }
...
//	//mf: logging
//	System.out.println("processSetListenerOpNode: view node:"+viewObject);
//	System.out.println("processSetListenerOpNode: set listener node:"+opNode);
//	System.out.println("processSetListenerOpNode: listener object:"+listenerObject);
...
//    //mf: logging
//    System.out.println("processSetListenerOpNode: statement:"+s);
//	System.out.println("processSetListenerOpNode: caller:"+caller);

    ListenerRegistration registration =
        listenerSpecs.getListenerRegistration(s);
    if (registration == null) {
      throw new RuntimeException(
          "Unexpected SetListener stmt: " + s + " @ " + caller);
    }
    
//    //mf: logging
//    System.out.println("processSetListenerOpNode: registration:"+registration.subsig);
...
//    //mf: logging
//    for(SootMethod hp:handlerPrototypes){
//    	System.out.println("processSetListenerOpNode: handler prototypes:"+hp);
//    }
    
    Set<SootMethod> handlers = Sets.newHashSet();
    computeConcreteHandlers(
        handlers, handlerPrototypes, Collections.singleton(listenerClass));
    
//    //mf: logging
//    for(SootMethod h:handlers){
//    	System.out.println("processSetListenerOpNode: handlers:"+h);
//    }
...
//      //mf: logging
//      System.out.println("processSetListenerOpNode: added edge to listener:"+h);
  ...
  NSetListenerOpNode createSetListenerAndProcessFlow(Local viewLocal,
    ...
    NSetListenerOpNode setListener = new NSetListenerOpNode(
        viewNode, listenerNode, callSite, isContextMenuSetListener, artificial);
  ...
  void processFlowFromSetListenerToEventHandlers(
	...
	//mf: logging
	//System.out.println("processFlowFromSetListenerToEventHandlers:view node:"+viewNode);
	//System.out.println("processFlowFromSetListenerToEventHandlers:statement:"+s);
	//System.out.println("processFlowFromSetListenerToEventHandlers:listener node:"+listenerNode);
...
//    //mf: logging
//    if(handlerPrototypes!=null){
//	    for(SootMethod hp:handlerPrototypes){
//	    	System.out.println("processFlowFromSetListenerToEventHandlers:handler prototypes:"+hp);
//	    }
//    }
...
//    //mf: logging
//	for(SootMethod h:handlers){
//		System.out.println("processFlowFromSetListenerToEventHandlers:handler:"+h);
//	}
    ...
    for (SootMethod h : handlers) {
      ...
      //mf: logging
      //System.out.println("processFlowFromSetListenerToEventHandlers:handler edge added:"+h);
  ...
  void computeConcreteHandlers(Set<SootMethod> handlers,
      Set<SootMethod> handlerPrototypes, NVarNode listenerNode) {
  ...
  void computeConcreteHandlers(Set<SootMethod> handlers,
      Set<SootMethod> handlerPrototypes, Set<SootClass> listenerTypes) {
    for (SootClass possibleListenerType : listenerTypes) {
      for (SootMethod prototype : handlerPrototypes) {
        ...
        SootClass matchedClass = hier.matchForVirtualDispatch(
  ...
  public NOpNode createAddMenuItemOpNode(Stmt s, Body b) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = ie.getMethod();
    String subsig = callee.getSubSignature();
    boolean isMenuAdd =
    ...
    //mf: find string id if possible
    if(menuAddIntSubSig.equals(subsig)){
    	if(ie.getArg(0) instanceof IntConstant){
    		IntConstant arg0 = (IntConstant) ie.getArg(0);
    		int stringId = arg0.value;
    		NStringIdNode stringIdNode = allNStringIdNodes.get(new Integer(stringId));
    		if(stringIdNode!=null){
    			String textReference = stringIdNode.getIdName();
    			Integer idValue = stringIdNode.getIdValue();
    			String textValue = xmlUtil.getStringValue(idValue);
    			inflMenuItem.setTextReference(textReference);
    			inflMenuItem.setTextValue(textValue);
    		}
    	}
	}
    //mf: find string id if possible
    if(menuAdd4IntSubSig.equals(subsig)){
    	if(ie.getArg(3) instanceof IntConstant){
    		IntConstant arg3 = (IntConstant) ie.getArg(3);
    		int stringId = arg3.value;
    		NStringIdNode stringIdNode = allNStringIdNodes.get(new Integer(stringId));
    		if(stringIdNode!=null){
    			String textReference = stringIdNode.getIdName();
    			Integer idValue = stringIdNode.getIdValue();
...
////  //mf: logging
//    System.out.println("menu node:"+menu);
//    System.out.println("menu item node:"+menuItem);
//    System.out.println("add view node:"+addMenuItem);
  ...
  public NOpNode createMenuInflateOpNode(Stmt s) {
    ...
    SootMethod callee = ie.getMethod();
    String sig = callee.getSignature();
    if (!(sig.equals(menuInflaterSig) || sig.equals(sherlockMenuInflaterSig))) {
      return null;
    }
    //mf: logging
    //System.out.println("menuId:"+ie.getArg(0).toString());
    //System.out.println("menu:"+ie.getArg(1).toString());
      ...
      if (concreteType.isApplicationClass()) {
        ...
        for (Value returnValue : returnValuesForGetView) {
          if (returnValue instanceof Local) {
            ...
            allNNodes.add(addView2);
//          //mf: logging
//          System.out.println("list view node:"+listViewNode);
//          System.out.println("list item node:"+listItemNode);
//          System.out.println("add view node:"+addView2);
            if (Configs.debugCodes.contains(Debug.LIST_ADAPTER_DEBUG)) {
...
//////mf: logging
//  System.out.println("find view by id node:"+custom);
//  System.out.println("view node:"+mView);
//  System.out.println("custom add view node:"+customAddView);
  }
...
//    MultiMapUtil.addKeyAndHashSetElement(tabHostNewTabSpecCalls, lhs, s);
  ...
  void processAddTabCall(NTabSpecNode tabSpecNode, Stmt addTab) {
    ...
    for (NVarNode tabIndicatorNode :
      ...
      allNNodes.add(addView2);
//////mf: logging
//  System.out.println("tab widget node:"+mTabWidgetNode);
//  System.out.println("tab indicator node:"+tabIndicatorNode);
//  System.out.println("add view node:"+addView2);
    }
...
//	//mf: logging
//	System.out.println("creating activity node for class:"+activityClass);
...
//    //mf: logging
//    System.out.println("activity node:"+activityClass);
    return activityNode;
  }
  ...
  public NMenuIdNode menuIdNode(Integer i) {
    ...
    x = new NMenuIdNode(i);
    //System.out.println("menu:"+x.getIdName());
  ...
  public NWidgetIdNode widgetIdNode(Integer i) {
    ...
    //mf: logging
    //System.out.println("widget:"+x.getIdName());
